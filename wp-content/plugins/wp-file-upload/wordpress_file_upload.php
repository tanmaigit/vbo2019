...
function wfu_enqueue_frontpage_scripts() {
	...
	wp_enqueue_script('wordpress_file_upload_script', WPFILEUPLOAD_DIR.'js/wordpress_file_upload_functions.js', array('jquery'));
...
function wordpress_file_upload_function($incomingfromhandler) {
	...
	if(isset($params['placements']) && $params['placements'] == 'list_on_fe'){
		// Import datatable
		wp_enqueue_style('wfu_datatables_style', WPFILEUPLOAD_DIR.'css/datatables/jquery.dataTables.min.css',false,'1.0','all');
		wp_enqueue_style('wfu_datatables_site_style', WPFILEUPLOAD_DIR.'css/datatables/site.css',false,'1.0','all');
		wp_enqueue_style('wfu_datatables_custom_style', WPFILEUPLOAD_DIR.'css/datatables/custom.css',false,'1.0','all');
		
		wp_enqueue_script('wfu_datatables_script', WPFILEUPLOAD_DIR.'js/jquery.dataTables.min.js', array('jquery'));
		// date range picker
		wp_enqueue_style('wfu_date_range_picker_bootstrap_css', WPFILEUPLOAD_DIR.'css/date_range_picker/bootstrap.css',false,'1.0','all');
		wp_enqueue_style('wfu_date_range_picker_daterangepicker_css', WPFILEUPLOAD_DIR.'css/date_range_picker/daterangepicker.css',false,'1.0','all');
		wp_enqueue_script('wfu_date_range_picker_moment_js', WPFILEUPLOAD_DIR.'js/date_range_picker/moment.min.js', array('jquery'));
		wp_enqueue_script('wfu_date_range_picker_daterangepicker_js', WPFILEUPLOAD_DIR.'js/date_range_picker/daterangepicker.js', array('jquery'));
		...
		<script type="text/javascript">
			var AJAX_CALL_URL_ON_LIST_FE="'.get_site_url().'/wp-admin/admin-ajax.php";
			var firstSubmittedData = '.json_encode($data['data']).';
			var columnMeta = '.json_encode($data['meta_data']).';
			jQuery(document).ready(function() {
				//console.log(firstSubmittedData);
				jQuery("#secondary").remove();
				jQuery("#primary").prop("class", "content-area egrid grid-100 tablet-grid-100 mobile-grid-100 tablet-push-30");
				registerDataTables("'.$tableId.'", columnMeta, firstSubmittedData, '.$data['files_total'].');
			});
		</script>';
		$table = $data['header_html'];
		return $style.$script.$table;
	}
