...
// tan.mai added
function wfu_ajax_action_callback_on_list() {
	// Get params
	$params = wfu_datatables_get_params();
	
	// Get data
	$data = wfu_datatables_get_data($params['filters'],$params['pagination'],$params['ordering']);
	...
	echo json_encode($dataTableResults);
	exit;
}
...
function wfu_ajax_action_callback() {
	if(isset($_REQUEST['action_type'])){
		if($_REQUEST['action_type'] == 'list_on_fe')
			echo wfu_ajax_action_callback_on_list();
		elseif($_REQUEST['action_type'] == 'download_file')
			wfu_download_file_on_fe($_REQUEST['upload_id']);
		elseif($_REQUEST['action_type'] == 'remove_file')
			wfu_remove_file_on_fe($_REQUEST['upload_id']);
		exit;
	}
