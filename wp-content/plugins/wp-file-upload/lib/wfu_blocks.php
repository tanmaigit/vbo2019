...
function wfu_prepare_textbox_block($params, $additional_params, $occurrence_index) {
	...
	if(!empty($_REQUEST['uploadid'])){
		$filepath = wfu_get_log_data_from_uploadid($_REQUEST['uploadid'], true, false);
		$filepath = !empty($filepath['log_data']->filepath) ? basename($filepath['log_data']->filepath) : "";
	}
	
	$textbox_item["line1"] = '<input type="text" id="'.$textfile.'" class="file_input_textbox"'.$styles.' value="'.$filepath.'" readonly="readonly" />';
...
function wfu_prepare_uploadform_block($params, $additional_params, $occurrence_index) {
	...
	// tan.mai
	if(!empty($_REQUEST['uploadid'])){
		$uploadform_item["line".$i++] = "\t".'<input type="hidden" id="loguploadid_'.$sid.'" name="loguploadid_'.$sid.'" value="'.$_REQUEST['uploadid'].'" />';
	}
...
function wfu_prepare_userdata_block($params, $additional_params, $occurrence_index) {
	...
	$defaul_userdatas = [];
	if(!empty($_REQUEST['uploadid'])){
		$defaul_userdatas = wfu_get_log_data_from_uploadid($_REQUEST['uploadid'], false, true);
		$defaul_userdatas = $defaul_userdatas['user_datas'];
	}
	...
	foreach ($params["userdata_fields"] as $userdata_key => $userdata_field) {
		...
		if ( $occurrence_index == 0 || $userdata_field["occurrence"] == $occurrence_index ) {
			//tan.mai
			if(!empty($defaul_userdatas[$userdata_field['label']]))
				$userdata_field['default'] = $defaul_userdatas[$userdata_field['label']];
