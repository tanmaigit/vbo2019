...
function coral_snowy_setup() {
	...
	add_editor_style( 'css/my-custom.css' );
	/*
	...
	register_nav_menus( array(
		...
		'social' => __( 'Social Links Menu', 'coral-snowy' ),
	) );
	/*
...
function coral_snowy_scripts() {
	...
	wp_enqueue_style( 'coral-snowy-my-custom', get_stylesheet_directory_uri() . '/css/my-custom.css' );
	wp_enqueue_style( 'coral-snowy-jquery.jscrollpane', get_stylesheet_directory_uri() . '/css/jquery.jscrollpane.css' );
	...
	wp_enqueue_script( 'jquery.mousewheel', get_template_directory_uri() . '/js/jquery.mousewheel.js', array('jquery'), '3.2' );
	wp_enqueue_script( 'jquery.jscrollpane', get_template_directory_uri() . '/js/jquery.jscrollpane.min.js', array('jquery'), '3.2' );
...
function coral_snowy_nivoslider() {
  ...
  echo '<div id="myslideshow" class="myslideshow grid-parent grid-100 tablet-grid-100 mobile-grid-100">
			...
			for ($i=1;$i<$total_slides+1;$i++) { 
				...
				$slide_text_title = get_theme_mod('slide_text_title'.$i,'0');
				if (0 != $slide_page) {
				...
				if ($slider_image) {
					if ($slide_title || !empty($slide_text_title)) { 
 ...
 */
function coral_snowy_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Get supported social icons.
	$social_icons = coral_snowy_social_links_icons();
...
add_filter( 'walker_nav_menu_start_el', 'coral_snowy_nav_menu_social_icons', 10, 4 );
 ...
 */
function coral_snowy_social_links_icons() {
	 ...
	 */
	return apply_filters( 'coral_snowy_social_links_icons', $social_links_icons );
}